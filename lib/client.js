...
function PickerInput(props) {
  ...
  const display = query ? props.items.map((it) => ({ it, score: Math.max(lcp(it.title.toLowerCase(), query), lcp(it.key.toLowerCase(), query)) })).sort((a, b) => b.score - a.score).map((x) => x.it) : [...props.items];
  React.useEffect(() => {
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "mcfg-pickerWrap", style: { flex: "1" } }, /* @__PURE__ */ React.createElement(
    "input",
    {
      ...
      onKeyDown: (e) => {
        if (e.key === "ArrowDown") {
          e.preventDefault();
          setIndex((i) => Math.min(i + 1, display.length - 1));
          return;
        }
        if (e.key === "ArrowUp") {
          e.preventDefault();
        ...
        if (e.key === "Enter") {
          e.preventDefault();
          if (display.length && index >= 0) confirm(display[index]);
        }
        if (e.key === "Escape") {
          e.preventDefault();
          setOpen(false);
  ...
  ), open ? /* @__PURE__ */ React.createElement("div", { className: "mcfg-picker", role: "listbox", "aria-label": props.ariaLabel }, display.length === 0 ? /* @__PURE__ */ React.createElement("div", { className: "mcfg-pickerEmpty" }, props.emptyText) : display.map((it, i) => /* @__PURE__ */ React.createElement(
    "div",
    {
      key: it.key,
